...
class NumberOfResponsesApproved extends CompletionCondition
{
    ...
    {
        return Response::forResource($activityInstance->id, $moduleInstance->id)->where('approved', true)->count() >= ( $settings['number_of_responses'] ?? 1);
    }

    public function percentage($settings, ActivityInstance $activityInstance, ModuleInstance $moduleInstance): int
    {
        $count = Response::forResource($activityInstance->id, $moduleInstance->id)->where('approved', true)->count();
    ...
    public function options(): Form
    {
        return \FormSchema\Generator\Form::make()->withField(
            \FormSchema\Generator\Field::number('number_of_responses')->setLabel('Number of Responses')
                ->setRequired(true)->setValue(1)->setHint('The number of approved responses')
    ...
    {
        return 'Number of responses approved';
    }
    ...
    {
        return 'Marks the module as complete when a number of responses have been approved';
    }
    ...
    {
        return 'number_of_responses_approved';
    }
